...
def process_single_file(eps, source_dir, output_dir, album_name, album_prefix, artist, titles, image_data):
    """Process a single audio file and save it with updated metadata."""
    ...
    if not os.path.exists(input_file):
        return f"Skipping {eps}.mp3 - File not found"
    ...
    audio.tags["TCOP"] = TCOP(encoding=3, text="Pocket Fm")  
    audio.tags["TPUB"] = TPUB(encoding=3, text="Pocket Fm")  
...
def process_audio_files(base_path, album_name, start_eps, end_eps):
    """Process and rename audio files with metadata tagging using multithreading."""
    ...
    num_files = end_eps - start_eps + 1
    progress_bar = tqdm(total=num_files, desc="Processing Files", unit="file")

    with ThreadPoolExecutor(max_workers=8) as executor:
        future_to_file = {
            executor.submit(process_single_file, eps, source_dir, output_dir, album_name, album_prefix, artist, titles, image_data): eps
            for eps in range(start_eps, end_eps + 1)
        }
